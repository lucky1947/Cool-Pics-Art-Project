 ...
 * <#uniqname(s)#> lmchow
 *
 * <#Description#> All member functions for class Line are implemented here.
 */
...
// Read line in form startPoint  endPoint  color
void Line::read(istream& ins) {
    Point startPoint;
    Point endPoint;
    Color color;
    
    ins >> startPoint >> endPoint >> color;
...
// Write line in form startPoint  endPoint  color
void Line::write(ostream& outs) {
    Point outsStart = getStart();
    Point outsEnd = getEnd();
    Color outsLineColor = getColor();
    
    outs << outsStart << " " << outsEnd << " " << outsLineColor << endl;
}
